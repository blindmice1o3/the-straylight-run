package com.jackingaming.thestraylightrun.accelerometer.game.gameconsole.game.scenes.commands.entities;

import com.jackingaming.thestraylightrun.accelerometer.game.gameconsole.game.scenes.entities.Entity;
import com.jackingaming.thestraylightrun.accelerometer.game.gameconsole.game.scenes.entities.Robot;

public class OpenRobotDialogEntityCommand
        implements EntityCommand {
    public static final String TAG = OpenRobotDialogEntityCommand.class.getSimpleName();

    private Entity entity;

    public OpenRobotDialogEntityCommand(Entity entity) {
        this.entity = entity;
    }

    @Override
    public boolean execute() {
        if (entity instanceof Robot) {
            ((Robot) entity).showRobotDialogFragment();
        }
        return false;
    }

    @Override
    public void setEntity(Entity entity) {
        this.entity = entity;
    }
}
