...
public class MochaLatte extends Drink {
    ...
    public boolean validate(CupImageView cupImageView,
        ...
        if (!cupImageView.isWhippedCream()) {
            return false;
        }
        ...
        if (shotCustom == -1) {
            ...
            if (cupImageView.getShots().size() == shotStandard) {
                if (syrupCustom == -1) {
                    // standard syrups
                    if (cupImageView.getSyrupsMap().containsKey(Syrup.Type.MOCHA)) {
                        int quantitySyrupMocha = cupImageView.getSyrupsMap().get(Syrup.Type.MOCHA).size();

                        if (quantitySyrupMocha == syrupStandard) {
                            return true;
                        }
                    }
                } else {
